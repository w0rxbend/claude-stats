//! The Weekly tab. See [`super::daily`]'s module doc -- this is the same
//! rendering over [`crate::application::report_source::ReportSource::weekly`]
//! instead.
...
const EMPTY: &str = "no weekly usage data available yet";
...
pub fn draw(
    ...
    draw_table_view(frame, body, "weekly usage", EMPTY, view, palette);
}
...
mod tests {
    ...
    fn fixture() -> TableView {
        TableView {
            title: "token usage by week".to_owned(),
            columns: vec![Column::left("Week"), Column::right("Cost (USD)")],
            rows: vec![vec!["2026-08-30".to_owned(), "$9.00".to_owned()]],
            totals: Some(vec!["Total".to_owned(), "$9.00".to_owned()]),
        }
    }
    ...
    fn a_loaded_report_shows_its_figures() {
        ...
        terminal
            .draw(|frame| draw(frame, frame.area(), 2, Some(&fixture()), &palette()))
        ...
        let screen: String = terminal
            ...
            .collect();
        assert!(screen.contains("2026-08-30"));
    }
    ...
    fn with_nothing_loaded_the_empty_message_is_shown() {
        let mut terminal = Terminal::new(TestBackend::new(60, 20)).expect("test backend");
        terminal
            .draw(|frame| draw(frame, frame.area(), 2, None, &palette()))
            .expect("draw succeeds");
        let screen: String = terminal
            .backend()
            .buffer()
            ...
            .map(ratatui::buffer::Cell::symbol)
            .collect();
        assert!(screen.contains("no weekly usage data"));
    }
