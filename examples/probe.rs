...
use claudetui::infrastructure::transcript::{locator::FileSystemCatalog, parser::TranscriptParser};

fn main() -> anyhow::Result<()> {
    let catalog = FileSystemCatalog::from_home()?;
    let selector = std::env::args().nth(1).map_or(SessionSelector::Active, |p| {
    ...
    println!("context      {} / {} = {:.1}%", s.context_fill().used(), s.context_window(), s.context_fill().percent());
    println!("severity     {:?}", s.context_fill().severity());
    println!("growth/turn  {:.0}", s.average_context_growth_per_turn());
    println!("compaction   {:?} (seen {})", s.compaction_distance(), s.compactions.len());
    println!("cost         {} ({}/turn, {:?}/h)", s.cost(), s.cost_per_turn(), s.burn_rate_per_hour().map(|u| u.to_string()));
    println!("cache        {:?}", s.cache_hit_ratio());
    println!("efficiency   {:?}", s.efficiency());
    println!("tools        {} calls, {} errors", s.tool_calls(), s.tool_errors);
    println!("kinds        {:?}", s.kind_counts);
    println!("files        {} touched, +{} -{}", s.files_touched(), s.lines_added, s.lines_removed);
    println!("thinking     {}  agents {}  skills {}", s.thinking_blocks, s.subagents, s.skills);
    println!("duration     {:?}", s.duration().map(|d| d.num_minutes()));
    println!("last error   {:?}", s.last_error);
    println!("recent       {:?}", s.recent_tools.iter().rev().take(5).map(claudetui::domain::activity::ToolEvent::label).collect::<Vec<_>>());
